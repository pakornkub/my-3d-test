...
export function demoScript(idea = 'อยากให้ตัวละครในออฟฟิศสะท้อนการทำงานของทีม agent จริง') {
  return [
    ...
    wait(7),
    ask('q1', 'choice', 'เมื่อวิศวกรกำลังใช้เครื่องมือ (เช่นแก้ไฟล์) คุณอยากเห็นชื่อไฟล์บนหัวตัวละครเลย หรือแค่บอกว่า "กำลังแก้โค้ด"?', {
      intro: 'อ่าน src/agents/director.js กับ src/bubbles.js มาแล้วครับ ตอนนี้ป้ายเหนือหัวรับข้อความสั้นได้อยู่แล้ว (SHORT ตัดที่ 90 ตัวอักษร) และ agent.tool ส่ง tool กับ summary มาให้ครบ ฉะนั้นเรื่องนี้ไม่ต้องแก้ contract อะไรเลย เหลือแค่ตัดสินใจว่าจะแสดงแค่ไหน',
      questions: [{
        id: 'Q1',
        header: 'Q1 · ป้ายเหนือหัวบอกละเอียดแค่ไหน',
        question: 'อันนี้กำหนดว่าฉากอ่านง่ายหรืออ่านยากตอนทีมทำงานพร้อมกันหกคน',
        multiSelect: false,
        options: [
          { label: '(ก) บอกชื่อไฟล์', description: 'เห็นทันทีว่าใครแตะไฟล์ไหน แต่ป้ายยาวและซ้อนกันเวลาเดินใกล้กัน' },
          { label: '(ข) บอกแค่ประเภทงาน', description: '"กำลังแก้โค้ด" / "กำลังอ่าน" ป้ายสั้น ฉากสะอาด รายละเอียดไปดูที่แท็บคุยแทน', recommended: true },
          { label: '(ค) สั้นปกติ ขยายเมื่อคลิก', description: 'คลิกตัวละครแล้วค่อยเห็นชื่อไฟล์เต็ม' },
        ],
        why: 'หกคนทำงานพร้อมกันแล้วชื่อไฟล์ยาว ๆ จะทับกันจนอ่านไม่ออก ส่วนชื่อไฟล์เต็มมีอยู่แล้วในแท็บคุย ไม่ได้หายไปไหน',
      }],
    }),
    wait(1.5),
