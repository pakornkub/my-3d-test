...
wss.on('connection', (ws) => {
  ...
  if (project) {
    ...
    for (const [id, job] of pipeline?.running ?? []) {
      ...
      unicast(ws, make('agent.start', { agent: job.agent, ticket: id, brief: `ใบ ${id} (กำลังทำอยู่)`, mode: 'implement' }));
      if (who !== job.agent) unicast(ws, make('agent.start', { agent: who, ticket: id, brief: `ใบ ${id} (${mode})`, mode }));
    }
